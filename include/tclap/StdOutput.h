...
inline void StdOutput::_shortUsage(CmdLineInterface& _cmd, std::ostream& os) const
{
    ...
    std::size_t secondLineOffset = progName.length() + 2;
    if (secondLineOffset > 75 / 2)
        secondLineOffset = 75 / 2;

    spacePrint(os, s, 75, 3, static_cast<int>(secondLineOffset));
}
...
inline void StdOutput::spacePrint(
    std::ostream& os, const std::string& s, int maxWidth, int indentSpaces, int secondLineOffset) const
{
    ...
    if ((len + indentSpaces > maxWidth) && maxWidth > 0) {
        ...
        while (start < len) {
            ...
            if (stringLen == allowedLen)
                while (stringLen >= 0 && s[static_cast<std::size_t>(stringLen + start)] != ' '
                    && s[static_cast<std::size_t>(stringLen + start)] != ','
                    && s[static_cast<std::size_t>(stringLen + start)] != '|')
                    stringLen--;
            ...
            for (int i = 0; i < stringLen; i++)
                if (s[static_cast<std::size_t>(start + i)] == '\n')
                    stringLen = i + 1;
            ...
            os << s.substr(static_cast<std::size_t>(start), static_cast<std::size_t>(stringLen)) << std::endl;

            // so we don't start a line with a space
            while (s[static_cast<std::size_t>(stringLen + start)] == ' ' && start < len)
                start++;
