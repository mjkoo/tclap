...
void parseOptions(int argc, char** argv)
{
    try {
        ...
        ValueArg<string> stest("", "stringTest", "string test", true, "homer", "string");
        cmd.add(stest);

        UnlabeledValueArg<string> utest("unTest1", "unlabeled test one", true, "default", "string");
        cmd.add(utest);

        UnlabeledValueArg<string> ztest("unTest2", "unlabeled test two", true, "default", "string");
        cmd.add(ztest);
        ...
        UnlabeledMultiArg<string> mtest("fileName", "file names", false, "fileNameString");
        cmd.add(mtest);
        //
